...
class MACDManager(Base, metaclass=Singleton):
    ...
    def openPositions(self) -> list:
        ...
        if self.tradingClient.clock.is_open:
            entryNum:int = min(50 - len(openedPositions), len(stockCandidates))
            for i in range(entryNum):
                order = self.tradingClient.openMACDPosition(stockCandidates[i], availableCash/(50-len(openedPositions)))
